...
def venues():
  data = []
  areas = db.session.query(Venue.city, State.name, State.id).\
    join(State).group_by(Venue.city, State.name, State.id).all()
    ...
    for venue in Venue.query.filter_by(city=city, state_id=state_id).all():
      area["venues"].append({
        ...
        "num_upcoming_shows": Show.query.filter(
          Show.venue_id == venue.id, Show.start_time > str(datetime.now())
        ).count()
      })
    data.append(area)
...
def show_venue(venue_id):
  ...
  data = {
    "id": venue.id,
    "name": venue.name,
    "genres": list(map(lambda g: g.name, venue.genres)),
